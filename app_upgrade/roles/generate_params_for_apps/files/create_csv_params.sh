...
nodeIP=$3
h2_server=$4
h2_server=${h2_server:=localhost}

h2=$(cat<<eof
java -cp /root/winning/tmts_miniservice/modules/ccp.server/public/lib/h2*.jar org.h2.tools.Shell -url jdbc:h2:tcp://${h2_server}//root/winning_source/h2/data/winning -user admin -password 11 -sql 
eof
)
echo $h2> /tmp/get.log
...
if [ $h2_server != "localhost" ];then
   scp ${h2_server}:${outPath}/{backend.csv,frontend.csv} ${outPath}/
fi
