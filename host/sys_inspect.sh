...
function getJDKStatus(){
    ...
    if [ $? -eq 0 ];then
        jvm_pid_list=$(jps -l | grep '/winning/app/winning' | awk '{print$1}')
        #echo "  PID  YGC(年轻代垃圾回收次数) YGCT(年轻代垃圾回收消耗时间)  FGC(老年代垃圾回收次数)  FGCT(老年代垃圾回收消耗时间)  GCT(GC总时间)"
        echo "  PID                JVM名称                YGC(年轻代垃圾回收次数)  FGC(老年代垃圾回收次数)  GCT(GC总时间) jvm运行时长(秒) GC运行占比 YGC平均间隔 FGC平均间隔"
        for pid in $(echo $jvm_pid_list)
        do
            proc_name=$(jps -l | grep $pid | cut -d '/' -f4)
            run_time=$(ps -eo pid,etime | grep $pid | awk '{print$2}')
            jstat=$(jstat -gc $pid | awk '{print $13,$14,$15,$16,$17}' | sed -n 2p)
            ygc=$(echo $jstat | awk '{print$1}')
            ygct=$(echo $jstat | awk '{print$2}')
            fgc=$(echo $jstat | awk '{print$3}')
            fgct=$(echo $jstat | awk '{print$4}')
            gct=$(echo $jstat | awk '{print$5}')
            run_sec=$(echo $run_time tr '-' ':' | awk -F: '{ total=0; m=1; } { for (i=0; i < NF; i++) {total += $(NF-i)*m; m *= i >= 2 ? 24 : 60 }} {print total}')
            gc_percent=$(awk 'BEGIN{printf "%.5f%%\n",('$gct'/'$run_sec')*100}')
            
            printf "%-9s%-45s%-22s%-20s%-15s%-14s%-14s%-10s%-12s\n" " $pid" "$proc_name" "$ygc" "$fgc" "$gct" "$run_sec" "$gc_percent" "$(expr $run_sec / $ygc)" "$(expr $run_sec / $fgc)"
        done
    fi
