...
main() {
    ...
    if [ "$2" == "install" ]; then
      ...
      ansible-playbook -i ../../$3 uninstall.yml -e "${params[*]}" --limit "$1" -t "$2"
    elif [ "$2" == "config" ]; then
      echo ansible-playbook -i ../../$3 config.yml -e "${params[*]}" --limit "$1" -t "$2"
      ansible-playbook -i ../../$3 config.yml -e "${params[*]}" --limit "$1" -t "$2"
    else
